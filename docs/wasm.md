...
```typescript
const answer = await Client.call(requestData);

const immediate = Client.callSync(requestData);
```

There is no `callAsync` here for the reason there is none for a typed call, which is that `call`
